...
Staffinfo.initColumn = function () {
    return [
        {field: 'selectItem', radio: true},
            ...
            {title: '是否管理员', field: 'adminName', visible: true, align: 'center', valign: 'middle'}
    ];
};
 ...
 * 设置员工管理员
 */
Staffinfo.setAdmin = function () {
	if (this.check()) {
        var ajax = new $ax(Feng.ctxPath + "/staffinfo/admin", function (data) {
            Feng.success("设置成功!");
            Staffinfo.table.refresh();
        }, function (data) {
 ...
 * 取消员工管理员
 */
Staffinfo.cancelAdmin = function () {
	if (this.check()) {
        var ajax = new $ax(Feng.ctxPath + "/staffinfo/notadmin", function (data) {
            Feng.success("设置成功!");
            Staffinfo.table.refresh();
        }, function (data) {
            Feng.error("设置失败!" + data.responseJSON.message + "!");
